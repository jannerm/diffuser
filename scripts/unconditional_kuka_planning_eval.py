...
def eval_episode(model, env, dataset, idx=0):
    ...
    if not osp.exists("uncond_samples/"):
        os.makedirs("uncond_samples/")

    writer = get_writer("uncond_samples/uncond_video_writer{}.mp4".format(idx))

    for frame in frames:
        writer.append_data(frame)

    np.save("uncond_samples/uncond_sample_{}.npy".format(idx), np.array(total_samples))
